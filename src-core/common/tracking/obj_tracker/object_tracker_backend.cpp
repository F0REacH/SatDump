...
namespace satdump
{
    ...
    std::vector<std::vector<ObjectTracker::SatAzEl>> ObjectTracker::getUpcomingPassPoints(std::vector<SatellitePass> &upcoming_satellite_passes, int time_steps)
    {
        std::vector<std::vector<SatAzEl>> upcoming_sat_pass_points;
        upcoming_passes_mtx.lock();
        if (tracking_mode == TRACKING_SATELLITE)
        {
            // find all relevant sat objects
            std::map<int, predict_orbital_elements_t*> norad_to_sat_object;
            // search registry for sat objects
            ...
            for(const auto& [norad, next_aos_time, next_los_time, max_elevation] : upcoming_satellite_passes)
            {
                ...
                upcoming_sat_pass_points.push_back(curr_sat_upcoming_pass_points);
            }
        ...
        upcoming_passes_mtx.unlock();
        return upcoming_sat_pass_points;
    }
}
